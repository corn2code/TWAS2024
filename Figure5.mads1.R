...
geneName = "mads1"  #args[1] #mads1 zcn26 arf34 late
...
# library("CMplot")
# 
# pdf(paste0("results/Manhathan/qqplot",geneName,".pdf"), height = 4, width = 4)
# par(mar=c(5.1,5.5,4.1,2.1))
# CMplot(gwas.dat[,c(1,2,3,8)],plot.type="q",box=FALSE,file="jpg",dpi=300,
...
mads69 <- genes.desc[which(genes.desc$`Gene Symbol` == "mads69"),1:6]
...
gTWAS.fdr <- ggplot() + 
  ...
  scale_y_continuous(limits = c(2, limite),
                     breaks = seq(2, limite, 2))
#+ labs(title="", subtitle=paste0(geneid))

gTWAS.fdr

# ggsave(plot=gTWAS.fdr, paste0("results/Manhathan/Manhathan_",geneName,".png"), width = 7, height = 4.5)
# ggsave(plot=gTWAS.fdr, paste0("results/Manhathan/Manhathan_",geneName,".pdf"), width = 7, height = 4.5)
...
my_y_title <- expression(paste(italic("mads1"), " (TPM)"))
...
#gwas.dat.RNA$CHROM[which(gwas.dat.RNA$log10 == max(gwas.dat.RNA$log10))]
...
proportion <- ggplot(result, aes(x = VlaCategory, y = proportion, fill = gene4)) +
  ...
  theme(axis.text.x = element_text(angle = 20, hjust = 1, size = 14), axis.title.x=element_blank(),legend.box.spacing = unit(0, "cm"),
...
#mads69
minW <- second.hitPOS-1000000
maxw <- second.hitPOS+1000000
...
mads69 <- second.hitPOS.window2 %>%
  filter(`Gene Symbol`=="mads69")
mads69pos2 <- ((mads69$start+mads69$end)/2)/1000000


second.hitPOS.window2$nothing <- NA
second.hitPOS.window2$nothing[grepl("Zm00001eb143080", second.hitPOS.window2$B73_V5)] <- "blue4"
second.hitPOS.window2$nothing[!grepl("Zm00001eb143080", second.hitPOS.window2$B73_V5)] <- "grey"

chr3_161301019.ld <- fread("chr3_161301019.ld", data.table = F)
second.hitPOS.snps.windowld <- merge(second.hitPOS.snps.window, chr3_161301019.ld , by.x=1, by.y=6)
...
hitPOS.snps.window <- gwas.dat %>%
  filter(CHROM==9, POS > minW & POS < maxw)
head(hitPOS.snps.window)

### Gene track
hitPOS.window <- genes.desc %>%
  filter(chr==9, start > minW &  end < maxw)
hitPOS.window2 <- hitPOS.window[,c(1,2,3,4,6,8,9,14)] 
head(hitPOS.window2)

hitPOS.window2$nothing <- NA
hitPOS.window2$nothing[grepl("Zm00001eb403750", hitPOS.window2$B73_V5)] <- "blue4"
hitPOS.window2$nothing[!grepl("Zm00001eb403750", hitPOS.window2$B73_V5)] <- "grey"

chr9_160163325.ld <- fread("chr9_160163325.ld", data.table = F)
hitPOS.snps.windowld <- merge(hitPOS.snps.window, chr9_160163325.ld , by.x=1, by.y=6)
...
zoomIncis <- ggplot() + 
  geom_point(data=hitPOS.snps.windowld, aes(POS/1000000, -log10(gene), fill=R2comp), shape = 21, alpha=0.5, size=4) +
  ylim(0,24) +
  scale_fill_manual(values = c("red4","orange3","yellow2","white"), 
                    breaks = c("veryHigh", "High", "medium","low")) +
  geom_point(data=hitPOS.window2[hitPOS.window2$strand=="+",],
             aes(x=((start+end)/2)/1000000, y=22, colour=nothing), shape="\u25BA", size=6) +
  geom_point(data=hitPOS.window2[hitPOS.window2$strand=="-",], 
             aes(x=((start+end)/2/1000000), y=20, colour=nothing), shape="\u25C4", size=6) +
  scale_color_manual(values = c("blue4", "grey")) +
  geom_hline(yintercept = -log10(0.05/nrow(gwas.dat)), linetype=2) + 
  #geom_vline(xintercept = second.hitPOS/1000000, linetype="dotted", color = "blue", linewidth=1.5) +
  #geom_vline(xintercept = mads69pos, linetype=2, color = "green", linewidth=1) +
...
######################################
plots <- align_plots(gTWAS.fdr, relation.plot, proportion, align = 'v', axis = 'l')
bottom_row <- plot_grid(plots[[2]], zoomIncis, zoomIn, rel_widths = c(.9,.9,.9),
                        nrow = 1, labels = c('B', 'C', 'D'),hjust = 0, vjust = 1, label_size = 18)
bottom_row2 <- plot_grid(plots[[3]],geneid.allele.plot, proportion2, geneid.allele.plot2, rel_widths = c(.9, .5,.9,.5),
...
ggsave(plot=fancyplots2, paste0("results/Manhathan/Fig4_",geneName,".svg"), width = 17, height = 13)
# 
